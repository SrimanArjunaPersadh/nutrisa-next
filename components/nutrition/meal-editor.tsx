 ...
 * daily use.
 *
 * WHAT IS NOT PULLED FORWARD: the "Add Item" search widget
 * (`logAddSearchWidget`, 3410). It needs ingredient search across the food
 * database, which is the dropdown rule and Phase 5's whole subject. Its absence
 * is why there is no base/added split here — the old app's `_logEditorIngs` /
 * `_logAddedIngs` pair exists only to track which rows came from that search.
 *
...
function IngredientEditor({
  ...
  const dirty =
    removed.size > 0 || qty.some((q, i) => q !== undefined && q !== baseQty(ings[i]));
...
function MacroEditor({ meal, onSave, onCancel, saving }: MealEditorProps) {
  ...
  /** Old app `updateLoggedMacros` (3382): parseFloat||0, then its own rounding. */
  function save() {
