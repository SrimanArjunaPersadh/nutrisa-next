...
### 21f. RESOLVED — the overwrite warning depended on a successful read

`components/library/meal-builder.tsx` derives `clash` from `library.meals`. If
that read failed, no overwrite warning appeared — but `saveCustomMeal` still
upserts on `name` and replaces the stored row. A saved meal definition could be
replaced without notice during an error state.

**Ruling (owner, 2026-08-06): warn, do not block.** The builder takes a required
`existingKnown` prop, true only in the two states where the read SUCCEEDED
(`ready` and `empty`). When it is false and a name has been typed, the builder
says the library could not be read and that saving will replace any meal of that
name.

Three properties of that choice, all deliberate:

- **It warns rather than refuses.** Blocking the save would lose the meal the
  user just built to a transient network blip — the same trade Phase 3 §6 made
  when it chose an Undo toast over a confirm dialog.
- **The prop is REQUIRED, not defaulted.** A defaulted `true` would let a future
  call site silently promise an overwrite check it cannot perform, which is the
  exact bug being fixed. A missing prop is now a type error.
- **`loading` counts as unknown too**, not just `error`. The builder card can be
  opened while the library is still reading, and an empty `meals` at that moment
  is indistinguishable from a genuinely empty library.

Tested in `tests/components/meal-builder.test.tsx`: the warning names the typed
meal, the save still goes through, and nothing is claimed before a name exists.
